    ...
    public class License : GLib.Object {
        ...
        public void get_license (string license_name) {
            try {
                ...
                this.license_text = this.regex.replace (license_data, license_data.length, 0, "");
                this.license_text = license_text.replace ("[year]", settings.get_string ("year"));
                this.license_text = license_text.replace ("[fullname]", settings.get_string ("fullname"));
                this.license_text = license_text.replace ("[e-mail]", settings.get_string ("email"));
                this.license_faq = match_info.fetch (1);
            } catch (GLib.Error e) {
                this.license_faq = e.message;
                this.license_text = e.message;
            }
        }
    }
